    ...
    static showWorkLogs(workLogs = []) {
        ...
        for (let j = 0; j <= workLogs.length; j++) {
            ...
            <tr class="work-log-item" data-worklog-id="${workLogs[j] ? workLogs[j].id : '0'}" data-issue-id="${workLogs[j] ? workLogs[j].id : '0'}">
                ...
                <td class="time-spend">${workLogs[j] ? convertMinutesToJira(workLogs[j].timeSpentSeconds / 60) : '<input type="text">'}</td>
                <td>${workLogs[j] ? escapeHTML(workLogs[j].issue.key) : '<input type="text">'}</td>
...
document.querySelector('#sign-in-form').addEventListener('submit', (e) => {
    ...
    try {
        ...
        let xhr = callApi(form.get('host'), form.get('user'), form.get('pass'), 'rest/api/2/project/', (response) => {
